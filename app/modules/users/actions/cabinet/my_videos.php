...
$Courses->videos->model->setSelectField($Courses->videos->model->getTableName() . '.*, courses_user_videos.*');
$Courses->videos->model->SetJoinImage('icon', 'video_icon');
$Courses->videos->model->setJoin('LEFT JOIN courses_user_videos ON courses_videos.video_id = courses_user_videos.uv_video_id');
$Courses->videos->model->columns_where->getTheme()->setValue($theme['theme_id']);
$Courses->videos->model->columns_where->getMain()->setValue(1);
$Courses->videos->model->addWhereCustom('courses_user_videos.uv_date_to >= CURRENT_DATE AND courses_user_videos.uv_user_id = '.$Main->user_info['user_id']);
